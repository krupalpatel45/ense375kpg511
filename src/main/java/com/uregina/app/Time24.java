...
public class Time24 
{
	 ...
	 * @param hours2 		hours of 24-hour time format (0:23)
	 * @param minutes	minutes of 24-hour time format (0:59)
	*/
	public Time24(int hours2, int minutes) throws InvalidTimeException
    {
		if(hours2<0||hours2>=24) throw new InvalidTimeException();
	...
	*/
	...
	{
		Time24 time = new Time24(hours, minutes);
		//Todo : add your code here
		if(am_pm == AmPm.am){
			hours = hours - 12;
		}
		if( (hours >=1 && minutes >= 00 && am_pm == AmPm.am) &&  (hours <=12 && minutes <= 59 && am_pm == AmPm.pm)){
			hours = hours + 12;

		}
		if( (hours >=1 && minutes >= 00) && (hours <=12 && minutes <= 59) && am_pm == AmPm.pm){
			hours = hours + 12;
